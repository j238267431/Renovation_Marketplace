...
function get_period_string($seconds)
{
    if ($seconds < 60)
    {
        ...
        $out = plural_form($seconds, ' ñåêóíäó', ' ñåêóíäû', ' ñåêóíä');
    }
    else if ($seconds < 60 * 60)
    {
        $period = Math.floor($seconds / 60);
        $out = plural_form($period, ' ìèíóòó', ' ìèíóòû', ' ìèíóò');
    }
    else if ($seconds < 60 * 60 * 24)
    {
        $period = Math.floor($seconds / (60 * 60));
        $out = plural_form($period, ' ÷àñ', ' ÷àñà', ' ÷àñîâ');
    }
    else if ($seconds < 60 * 60 * 24 * 30)
    {
        /* Îêðóãëåíèå ââåðõ äëÿ áîëåå êîððåêòíîãî ïîêàçà ÷èñëà äíåé */
        $period = /*floor*/Math.ceil($seconds / (60 * 60 * 24));
        $out = plural_form($period, ' äåíü', ' äíÿ', ' äíåé');
    }
    else if ($seconds < 60 * 60 * 24 * 30 * 12)
    {
        $period = Math.floor($seconds / (60 * 60 * 24 * 30));
        $out = plural_form($period, ' ìåñÿö', ' ìåñÿöà', ' ìåñÿöåâ');
    }
    else
    {
        $period = Math.floor($seconds / (60 * 60 * 24 * 30 * 12));
        $out = plural_form($period, ' ãîä', ' ãîäà', ' ëåò');
    }
...
function calculate_time_ago()
{
    ...
    if (localStorage['time_offset'])
    {
        ...
        $('.time_ago').each(function (i, item)
        {
            ...
            $(item).html(get_period_string(time) + ' íàçàä');
        });
    }
}
...
function get_notify()
{
    ...
    $.get('/ajax/?section=notifications&time='+expire, function(data){
        ...
        if(data['new_pm_contacts'])
        {
            ...
            {
                $('.all_contacts a').html('Âñåãî êîíòàêòîâ: ' + data['new_pm_contacts'].length);
            ...
            $('.contacts').popover({html: true, trigger: 'manual', placement: 'auto', title: 'Ñîîáùåíèå', content: block, container: 'body'});
        }else
        {
       ...
       */
        if(data['notice'])
        {
            ...
            if (data['notice']['message'])
            {
                ...
                if (total > 1) content += '<div class="small indent-t10"><a href="' + data['notices_url'] + '">Âñåãî óâåäîìëåíèé: ' + total + '</a></div>';

                $('.notices_block').removeClass('hidden');
                $('.notices').popover({html: true, trigger: 'manual', placement: 'auto', title: 'Óâåäîìëåíèå', content: content, container: 'body'});
            }
            else
            {
...
$(function ()
{
    ...
    $('.note').popover({html: false, trigger: 'manual', placement: 'bottom', content: 'Íàæìèòå, ÷òîáû äîáàâèòü çàìåòêó.', container: 'body'});
    ...
    {
        // Äëÿ ìîáèëüíûõ ïî êëèêó ïîêàçûâàåì çàìåòêó, íî íå îòêðûâàåì modal.
    ...
    $('#note_save').on('click',function(){
        ...
        $.post(url,{'action':'add_note','note':note, 'fk': fk},
            function(){
                ...
                $('.note').popover({html: true, trigger: 'manual', placement: 'bottom', content: 'Íàæìèòå, ÷òîáû äîáàâèòü çàìåòêó.'});
    ...
    // Ïîäñòðàèâàåì êëàññ ñàéäáàðà, åñëè âñå âëîæåííûå áëîêè ñêðûâàþòñÿ íèæå lg
    ...
    /*
     * Ñîõðàíåíèå çíà÷åíèé ïîëåé ââîäà è ÷åêáîêñîâ
     * */
    ...
    // Íà ãëàâíîé ñòðàíèöå ñòàôèì ôîêóñ â ôîðìó çàêàçà
    //$('.page_layer-index .form-control').focus();

    // Ïîëå äëÿ ââîäà öåëîãî ÷èñëà
...
function init_spoiler()
{
    ...
    $('.spoiler').each(function(i,item){
        if(item.scrollHeight - item.offsetHeight > 3) // 3px çàïàñ, ÷òîáû ó÷èòûâàëèñü îòñòóïû è border
        {
            $(item).after('<a href="" class="dropdown-toggle expand_spoiler">×èòàòü äàëüøå</a>');
        }else
        {
