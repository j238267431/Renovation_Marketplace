    ...
    <div class="wrapper tab-content">
        <div class="clearfix tab-pane fade show active" id="tab_pane-main">
            <div class="page_content no_sidebar">
                <form class="form" action="{{route('tasks.response.update', ['task'=>$task, 'response'=>$response])}}" method="POST" enctype="multipart/form-data" autocomplete="off">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="row">
                            <div class="col">
                                        ...
                                        <div class="form-group row bootstrap_css category_select">
                                            <div class="form-group col-sm-6 mb-sm-0">
                                                ...
                                                <select name="company_id" id="company" class="form-control">
                                                    <option disabled value="No" @if($task->company_id) '' @else selected @endif>Выберите компанию</option>
                                                    @foreach($companies as $company)
                                                        <option value="{{$company->id}}" @if($respondedCompany->id==$company->id) selected @else""@endif>{{$company->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                ...
                                @if($task->budget)
                                    ...
                                    <fieldset class="form-group">
                                        ...
                                        <div class="input-group amount indent-t20" @if(old('category_toggle') == 0) style="display: none"@endif id="set_budget">
                                            <input type="number" class="form-control" id="budget" name="budget" placeholder="Сумма в руб." value="{{$response->price}}" min="1" autocomplete="off" validate="true">
                                ...
                                <div class="form-group indent-b0">
                                    ...
                                    <textarea name="description" id="description" cols="30" rows="10" placeholder="Комментарии к отклику га заказ">{!! $response->description !!}"</textarea>
                                </div>
                                    ...
                                    <div class="attachment file buttons">
                                        <div id="files_form">
                                            <a href="" class="btn btn-secondary btn-file upload">
                                                ...
                                                Прикрепить файлы
                                                <input type="file" class="uploader" name="file[]" multiple="multiple" value="{{$response->file}}">
                                            </a>
                                        </div>
                                    </div>
                                </div>
